import { Globalize, PrintDocument, escapeHtml } from '@mescius/wijmo';
import { CellType, HeadersVisibility } from '@mescius/wijmo.grid';
 ...
 */
function renderTable(flex, filteredData) {
    // header
    const head = [
        '<thead>',
    ...
    const body = [
        ...
        filteredData.map((row, index) => {
            flex.cells.rows[index]._data = row;
            return renderRow(flex.cells, index);
        }).join(''),
        '</tbody>',
    ].join('');
    // table
 ...
 * @param filteredData - "soft refreshed" data
 */
export function print(grid, filteredData) {
 ...
 * @param filteredData - "soft refreshed" data
 */
export function pdf(grid, filteredData) {
