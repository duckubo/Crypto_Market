...
import { SelectionMode } from '@mescius/wijmo.grid';
import { FlexGridDetailProvider } from '@mescius/wijmo.grid.detail';
import { FlexGrid, FlexGridColumn } from '@mescius/wijmo.react.grid';
// material-ui
import { PlaylistAdd, FilterList } from '@mui/icons-material';
// components
...
class Grid extends React.Component {
    constructor() {
        ...
        this.handleInitialized = gridPanel => {
            if (gridPanel.columnHeaders.rows) {
                gridPanel.columnHeaders.rows.defaultSize = 78;
            }
    ...
    componentDidUpdate() {
        this.flexGridRef.control.rows.defaultSize = this.props.settings.rowHeight;
    }
    render() {
        ...
        return (<div className="grid">
            ...
            <FlexGrid allowSorting="None" isReadOnly className="flex-grid" ref={this.setFlexGridRef} selectionMode={SelectionMode.Row} autoGenerateColumns={false} frozenRows={settings.isFreezeFirstRow ? 1 : 0} frozenColumns={settings.isFreezeFirstCol ? 1 : 0} formatItem={this.handleFormatItem} initialized={this.handleInitialized} updatingView={this.handleUpdatingView}>
                {columnsData.map((column, index) => (column.visible &&
